...
def part2():
    ...
    for line in lines:
        range1 = line.split(',')[0].split('-')
        range2 = line.split(',')[1].split('-')
        if int(range1[0]) <= int(range2[0]) <= int(range1[1]) or int(range1[0]) <= int(range2[1]) <= int(range1[1]) or int(range2[0]) <= int(range1[0]) <= int (range2[1]) or int(range2[0]) <= int(range1[1]) <= int(range2[1]):
            completeOverlap += 1
